...
@Component({
  ...
  templateUrl: './scans-client.component.html',
  styleUrl: './scans-client.component.scss'
})
export class ScansClientComponent implements OnInit{
  ...
  getAuditsForClient() {
    ...
    this.authService.authenticatedUser$.pipe(
      switchMap(user => this.auditService.findByClient(user.id))
    ).subscribe((res: any) => {
